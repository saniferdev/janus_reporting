    ...
    if(isset($_GET['date_debut'])){
        ...
        if($_SESSION['depot'] == "SEC"){
            ...
            /*$tsql = "SELECT DISTINCT(DO_Piece) FROM (SELECT
                ...
                INNER JOIN facture_ligne ON facture_entete.DO_Piece = facture_ligne.DO_Piece
                INNER JOIN article ON facture_ligne.AR_Ref = article.AR_Ref
                WHERE article.DL = '".$_SESSION['depot']."' AND facture_entete.statut = 0 AND facture_entete.DO_Provenance = 0)
                AS TabENT
                WHERE DO_Date = '" . $user_current_date. "'";*/
            ...
            $tsql = "SELECT DISTINCT(DO_Piece),DO_Ref,lineStat,CT_Intitule,CT_Adresse,DO_Coord01,DO_Coord02,DO_Coord03,DO_Coord04,entDate,DO_Tiers,prepa FROM (SELECT
                    ...
                    AND dbo.facture_ligne.statut != 0 AND dbo.facture_entete.DO_Provenance = 0)
                    AS tabEnt
                                            ...
                                            <div class="card-header" id="headingThree">
                                                <h2 class="mb-0">
                                                    <button style="width: 1130px;text-align: left;font-size: 13px;" class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#{{ data.DO_Piece }}" aria-expanded="false" aria-controls="{{ data.DO_Piece }}">
                                                        ...
                                                        <span class="col-md-2"><?php echo  $user_current_date; ?></span>
                                                        <span class="col-md-1">
