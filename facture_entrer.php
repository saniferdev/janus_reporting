    ...
    if(isset($_GET['date_debut'])){
        ...
        if($_SESSION['depot'] == "SEC"){
            /*$tsql = "SELECT DISTINCT(DO_Piece),DO_Tiers,CT_Intitule FROM (SELECT
                ...
                LEFT JOIN dbo.historique_entre ON  dbo.historique_entre.DO_Piece = dbo.facture_entete.DO_Piece
                WHERE dbo.historique_entre.DO_Piece IS NULL
                AND ".$_SESSION['where']." AND facture_entete.DO_Type IN (6,7,30) AND facture_entete.DO_Provenance = 0)
                AS TabENT
            ...
            ,CT_Adresse,DO_Coord01,DO_Coord02,DO_Coord03,DO_Coord04,entDate,DO_Tiers,prepa FROM (SELECT
                    ...
                    LEFT JOIN dbo.historique_entre ON  dbo.historique_entre.DO_Piece = dbo.facture_entete.DO_Piece
                    WHERE
                    dbo.historique_entre.DO_Piece IS NULL AND ".$_SESSION['where']." 
        ...
        if($_SESSION['depot'] == "SEC"){
           /* $tsql = "SELECT DISTINCT(DO_Piece),DO_Tiers,CT_Intitule FROM (SELECT
                ...
                INNER JOIN dbo.client ON dbo.facture_entete.DO_Tiers = dbo.client.CT_Num
                LEFT JOIN dbo.historique_entre ON  dbo.historique_entre.DO_Piece = dbo.facture_entete.DO_Piece
                WHERE dbo.historique_entre.DO_Piece IS NULL AND ".$_SESSION['where']."  AND facture_entete.DO_Type IN (6,7,30) AND facture_entete.DO_Provenance = 0)
                AS TabENT
                WHERE CAST(DO_Date AS date) = '" . $user_current_date. "'";*/
                            $tsql = "SELECT DISTINCT TOP 50  (DO_Piece),DO_Ref,lineStat,
            CASE 
                ...
                ELSE CT_Intitule
            END AS CT_Intitule
            ,CT_Adresse,DO_Coord01,DO_Coord02,DO_Coord03,DO_Coord04,entDate,DO_Tiers,prepa FROM (SELECT TOP 25
                    dbo.facture_ligne.DO_Piece,
                    dbo.facture_entete.statut AS entStat,
                    dbo.facture_ligne.DO_Ref,
                    dbo.facture_entete.DO_Date AS entDate,
                ...
                <div class="col-md-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <form method="get" action="facture_entrer.php" class="form-horizontal" enctype="multipart/form-data">
            ...
            $scope.insertEntrer = function(){
                ...
                $http.post("insertHistory.php",{'do_piece':$scope.numFacture})
                    .success(function(data){
                        ...
                        console.log(data);
                        window.location.href = 'facture_entrer.php';
                    });
